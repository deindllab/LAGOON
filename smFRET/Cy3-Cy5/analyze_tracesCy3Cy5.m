...
norm_Cy3_1 = mean(handles.U(1,current_trace_num).Cy3_1(x1:x2));
norm_Cy5_1 = mean(handles.U(1,current_trace_num).Cy5_1(x1:x2));
handles.U(1,current_trace_num).Cy3_1 = handles.U(1,current_trace_num).Cy3_1 - norm_Cy3_1;
handles.U(1,current_trace_num).Cy5_1 = handles.U(1,current_trace_num).Cy5_1 - norm_Cy5_1;
handles.U(1,current_trace_num).FRET_1 = handles.U(1,current_trace_num).Cy5_1./(handles.U(1,current_trace_num).Cy5_1 + handles.U(1,current_trace_num).Cy3_1);
%% Recalculate FRET
...
if (~handles.dist_calculated)  %check to see if calculation to distance domain has already been performed
	...
	for i=1:handles.num_traces
		for j=1:handles.num_fr
            ...
            dist_FRET(j,i) = 5.9*(1/FRET_traces(j,i)-1)^(1/6);  %Ro = 5.9 for Cy3_1/Cy5_1
		end
	end
...
dyesum = handles.U(1,current_trace_num).Cy3_1 + handles.U(1,current_trace_num).Cy5_1;
[rows] = find(dyesum < threshold);
handles.U(1,current_trace_num).FRET_1 = handles.U(1,current_trace_num).Cy5_1./dyesum;
handles.U(1,current_trace_num).FRET_1(rows,1) = 0;

handles.U(1,current_trace_num).fretthresh = threshold;
...
%handles structure from the gui to get latest variables
    ...
    Cy3_1 = handles.U(1,current_trace_num).Cy3_1;
    Cy5_1 = handles.U(1,current_trace_num).Cy5_1;
    threshold = handles.U(1,current_trace_num).fretthresh;
    
    Y_1 = handles.U(1,current_trace_num).FRET_1;
    Y_1((Cy3_1+Cy5_1)<threshold) = 0;
    ...
    if ishandle(handles.figure)
        figure(handles.figure);
        handles.Cy3plot.XData = X;
        handles.Cy3plot.YData = Cy3_1;
...
%             handles.FRETplot.Color(4) = 0.25;
    ...
    %The figure doesn't yet exist
        ...
        handles.Cy3plot = plot(X,Cy3_1,'g','LineWidth',2);
        hold on
